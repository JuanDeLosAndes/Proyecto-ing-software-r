...
def _agregar_celda(horario: dict, hora: str, dia: str, payload: dict):
    """SRP: inserta una celda en el dict de horario."""
    if hora and dia:
        ...
        if est:
            for ins in session.exec(
                ...
                _agregar_celda(horario, grupo.hora, grupo.dia, {
                    "materia": nombre_mat, "salon": sal.nombre if sal else "Sin salon",
                    "num_grupo": grupo.num_grupo, "info_extra": f"Profesor(a): {nombre_prof}",
                    "salon_nombre": sal.nombre if sal else "Sin salon",
                    "prof_nombre": nombre_prof, "sesion": 1
                })
                _agregar_celda(horario, grupo.hora2, grupo.dia2, {
                    "materia": nombre_mat, "salon": sal2.nombre if sal2 else "Sin salon",
                    "num_grupo": grupo.num_grupo, "info_extra": f"Profesor(a): {nombre_prof}",
                    "salon_nombre": sal2.nombre if sal2 else "Sin salon",
                    "prof_nombre": nombre_prof, "sesion": 2
                })
        ...
        if prof:
            for g in session.exec(select(Grupo).where(Grupo.id_profesor == prof.id)).all():
                ...
                _agregar_celda(horario, g.hora, g.dia, {
                    ...
                    "id_grupo": g.id, "num_grupo": g.num_grupo, "info_extra": extra,
                    "salon_nombre": sal.nombre if sal else "Sin salon",
                    "prof_nombre": prof.nombre, "sesion": 1
                })
                _agregar_celda(horario, g.hora2, g.dia2, {
                    ...
                    "id_grupo": g.id, "num_grupo": g.num_grupo, "info_extra": extra,
                    "salon_nombre": sal2.nombre if sal2 else "Sin salon",
                    "prof_nombre": prof.nombre, "sesion": 2
                })
